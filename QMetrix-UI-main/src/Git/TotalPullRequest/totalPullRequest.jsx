...
const TotalPullRequests = () => {
  ...
  const [getTotalMergeRequestsData, setGetTotalMergeRequestsData] = useState([]);
  ...
  useEffect(() => {
    ...
    if (gitData) {
      setGetTotalMergeRequestsData(gitData.totalPRs);
    }
  }, [jiraData, gitData]);
  ...
  const SprintLabels = getTotalMergeRequestsData?.totalPullRequests?.map((item) => item.name) ?? [];
  const SprintDatasetData =
    getTotalMergeRequestsData?.totalPullRequests?.map((item) => item.count) ?? [];
  const TeamLabels =
    getTotalMergeRequestsData?.getTotalPullRequestsByDev?.map((item) => item.name) ?? [];
  const TeamDataSets =
    getTotalMergeRequestsData?.getTotalPullRequestsByDev?.map((item) => item.count) ?? [];
  const releaseLabels =
    getTotalMergeRequestsData?.totalPullRequests?.map((item) => item.name) ?? [];
                ...
                <>
                  {selectedChartBySprint === 'barGraph' && (
                    <BarChart
                      ...
                      datasetLabel={
                        repoConnector === 'GitLab' ? 'Total Merge Requests' : 'Total Pull Requests'
                      }
                      height={300}
                      width={500}
        ...
        {selectedValue?.value === APP_STRINGS.VALUE_RELEASE && (
          <div className="bg-white dark:bg-[#182433] p-4 rounded-md shadow-md">
            <div className="flex flex-wrap justify-between items-center mb-4">
              <h4 className="text-lg font-semibold text-black dark:text-gray-300">
                {repoConnector === 'GitLab'
                  ? 'Total MR Per Release'
                  : repoConnector === 'GitHub'
                  ? 'Total PR Per Release'
                  : 'Total PR Per Release'}
              </h4>
              <div className="w-auto">
...
export default TotalPullRequests;
