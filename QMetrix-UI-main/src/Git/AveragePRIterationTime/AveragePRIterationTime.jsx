...
const AveragePRIterationTime = () => {
      ...
      tooltip: {
        callbacks: {
          label: function (tooltipItem) {
            ...
            return `${label}: ${tooltipItem.raw} hrs`;
          },
        },
      },
    },
  ...
  const averagePRIterationTime = getIterationTimeData?.AveragePRsIterationTime ?? 0;
  const SprintLabels =
    getIterationTimeData?.averagePRIterationTimePerSprint?.map((item) => item.sprint) ?? [];
  const SprintDatasetData =
    getIterationTimeData?.averagePRIterationTimePerSprint?.map((item) => item.iterationTime) ?? [];
  const TeamLabels =
    getIterationTimeData?.averagePRIterationTimeByDev?.map((item) => item.name) ?? [];
  const TeamDataSets =
    getIterationTimeData?.averagePRIterationTimeByDev?.map((item) => item.iterationTime) ?? [];
  const releaseLabels =
    getIterationTimeData?.averagePRIterationTimePerRelease?.map((item) => item.release) ?? [];
  const releaseDatasetData =
    getIterationTimeData?.averagePRIterationTimePerRelease?.map((item) => item.iterationTime) ?? [];
            ...
            <div className="flex flex-wrap justify-between items-center mb-4">
              <h4 className="text-lg font-semibold text-black dark:text-gray-300">
                {repoConnector === 'GitLab'
                  ? 'Total Average MR Iteration Time'
                  : repoConnector === 'GitHub'
                  ? 'Total Average PR Iteration Time'
                  : 'Total Average PR Iteration Time'}
              </h4>
            </div>
            <div style={{ height: '300px' }} className="flex justify-center relative top-12">
              <ReactSpeedometer
                maxValue={100}
                value={averagePRIterationTime}
                needleColor="#7367f0"
        ...
        {selectedValue?.value === APP_STRINGS.VALUE_SPRINT && (
          <div className="bg-white dark:bg-[#182433] p-4 rounded-md shadow-md">
            <div className="flex flex-wrap justify-between items-center mb-4">
              <h4 className="text-lg font-semibold text-black dark:text-gray-300">
                {repoConnector === 'GitLab'
                  ? 'Avg MR Iteration Time Per Sprint'
                  : repoConnector === 'GitHub'
                  ? 'Avg PR Iteration Time Per Sprint'
                  : 'Avg PR Iteration Time Per Sprint'}
              </h4>
        ...
        {(selectedValue?.value === APP_STRINGS.VALUE_SPRINT || selectedValue?.value === APP_STRINGS.VALUE_RELEASE) && (
          <div className="bg-white dark:bg-[#182433] p-4 rounded-md shadow-md">
            <div className="flex flex-wrap justify-between items-center mb-4">
              <h4 className="text-lg font-semibold text-black dark:text-gray-300">
                {repoConnector === 'GitLab'
                  ? 'Avg MR Iteration Time By Dev'
                  : repoConnector === 'GitHub'
                  ? 'Avg PR Iteration Time By Dev'
                  : 'Avg PR Iteration Time By Dev'}
              </h4>
              <div className="w-auto">
                {TeamLabels.length > 0 && (
        ...
        {selectedValue?.value === APP_STRINGS.VALUE_RELEASE && (
          <div className="bg-white dark:bg-[#182433] p-4 rounded-md shadow-md">
            <div className="flex flex-wrap justify-between items-center mb-4">
              <h4 className="text-lg font-semibold text-black dark:text-gray-300">
                {repoConnector === 'GitLab'
                  ? 'Avg MR Iteration Time Per Release'
                  : repoConnector === 'GitHub'
                  ? 'Avg PR Iteration Time Per Release'
                  : 'Avg PR Iteration Time Per Release'}
              </h4>
              <div className="w-auto">
...
export default AveragePRIterationTime;
