...
const TrafficSourceChart = ({ getTaskCountValue }) => {
  ...
  const filteredIssesTypes = issesTypes.filter(
    (item) => item.name.toLowerCase() !== 'done' && item.name.toLowerCase() !== 'closed',
  );
  const labels = filteredIssesTypes ? filteredIssesTypes.map((item) => item.name) : [];
  const series = filteredIssesTypes ? filteredIssesTypes.map((item) => item.count) : [];
