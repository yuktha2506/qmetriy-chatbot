...
export const DeploymentFrequency = ({ onAverageChange }) => {
  ...
  const chartData = (getDoraData.dFTrend || []).map((item) => ({
    day: item.name || 'Unknown',
    deployment: Number(item.avgDeploymentsPerDay) || 0,
    deploymentColor: "#84A9FF"
  }));
 const averageDeploymentFrequency = useMemo(() => {
    if (!chartData.length) return 0;
    return chartData.reduce((sum, item) => sum + item.deployment, 0) / chartData.length;
  }, [chartData]);

  useEffect(() => {
    if (onAverageChange) {
      onAverageChange(averageDeploymentFrequency);
    }
  }, [averageDeploymentFrequency, onAverageChange]);
  return (
    <div>
          ...
          <div className="flex justify-between items-center mb-2">
            <div className="flex items-center">
              <h2 className="text-lg font-semibold text-[#0A2342] dark:text-[#e5e7eb] mr-4">
                {`Deployment Frequency by ${defectDensityValue}s`}
              </h2>
            </div>
