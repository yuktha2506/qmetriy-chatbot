...
export const MeanTimeRecovery = ({ onAverageChange }) => {
  ...
  const chartData = (getDoraData.mttrTrend || []).map((item) => ({
    day: item.name || 'Unknown',
    recovery: Number(item.mttr) || 0,
    recoveryColor: "#84A9FF"
  }));


  const averageMeanTimeRecovery = useMemo(() => {
    if (!chartData.length) return 0;
    return chartData.reduce((sum, item) => sum + item.recovery, 0) / chartData.length;
  }, [chartData]);

  useEffect(() => {
    if (onAverageChange) {
      onAverageChange(averageMeanTimeRecovery);
    }
  }, [averageMeanTimeRecovery, onAverageChange]);
  return (
    <div>
          ...
          <div className="flex justify-between items-center mb-2">
            <div className="flex items-center">
              <h2 className="text-lg font-semibold text-[#0A2342] dark:text-[#e5e7eb] mr-4">
                {`Mean Time Recovery by ${defectDensityValue}s`}
              </h2>
            </div>
